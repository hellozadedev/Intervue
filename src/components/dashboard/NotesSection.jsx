  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        <Label htmlFor={`${noteType}-notes-textarea`} className="sr-only">Add new note for {title}</Label>
        <Textarea
          id={`${noteType}-notes-textarea`}
          placeholder={`Type your ${noteType.toLowerCase().includes('pre') ? 'pre-interview' : 'post-interview'} notes here...`}
          value={newNote}
      ...
      {notes && notes.length > 0 ? (
        <ScrollArea className="h-[200px] w-full rounded-md border p-4 bg-background shadow-inner">
          <div className="space-y-3">
            {notes.slice().sort((a, b) => new Date(b.timestamp) - new Date(a.timestamp)).map((noteItem) => (
              <div key={noteItem._id || noteItem.timestamp} className="p-3 rounded-md bg-secondary/50 shadow-sm">
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  {format(new Date(noteItem.timestamp), "MMM d, yyyy 'at' h:mm a")}
                </p>
              </div>
            ))}
          </div>
